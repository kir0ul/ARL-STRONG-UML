...
let sounds = {
  bg: {
    fileName: "audio/ambient.mp3",
    file: new Audio("audio/ambient.mp3"),
    shouldLoop: true,
  },
  pick: {
    fileName: "audio/coin-drop.mp3",
    file: new Audio("audio/coin-drop.mp3"),
    shouldLoop: false,
  },
  coins_drop: {
    fileName: "audio/coins-drop.mp3",
    file: new Audio("audio/coins-drop.mp3"),
    shouldLoop: false,
  },
  gold_sack: {
    fileName: "audio/gold_sack.wav",
    file: new Audio("audio/gold_sack.wav"),
    shouldLoop: false,
  },
};
...
class Player {
  ...
  pickTarget() {
    ...
    if (pickedObstacle.length == 0 || pickedObstacle[0].isPicked) return false;
...
function resizeInstructionsModal() {
  $instructionsModal[0].style.setProperty(
    "width",
    `${document.querySelector("#map-container").offsetLeft / parseFloat(getComputedStyle(document.documentElement).fontSize) - 8}rem`,
    "important",
  );
  $instructionsModal[0].style.setProperty("height", `auto`);
  updateScrollingPosition(human.x, human.y);
}

function resizeInstructionsModalVideo() {
  document.querySelector("#instructions-modal video").style.height =
    document.querySelector("body").clientHeight * 0.8 -
    2 *
      parseFloat(
        getComputedStyle(document.querySelector("#instructions-modal"))
          .paddingTop,
      ) -
    document.querySelector("#instructions-heading").clientHeight -
    parseFloat(
      getComputedStyle(document.querySelector("#instructions-heading"))
        .paddingBottom,
    ) -
    document.querySelector("#instructions-modal-content .userInputButtons")
      .clientHeight -
    parseFloat(
      getComputedStyle(
        document.querySelector("#instructions-modal-content .userInputButtons"),
    ...
    "px";
  document.querySelector("#instructions-modal").style.height =
    document.querySelector("body").clientHeight * 0.8 + "px";
}
...
function showInstructions2() {
  ...
  setTimeout(() => {
    ...
    $("#instructions-content").html(
      `<video controls style="width:100%;"><source src='video/tutorial_1.webm' type='video/webm' /></video>`,
    );
    ...
    vid.onloadedmetadata = () => {
      ...
      $("#instructions-content > video").on("pause", () =>
        pausePlaying(vid.duration - 1),
      );
    };
...
function showInstructions12() {
  ...
  setTimeout(() => {
    ...
    $("#instructions-content").css("display", "initial");
    $("#instructions-content").html(
      `Let's practice picking up the gold coins you just discovered while practicing movement. Move to the center of both coins and press spacebar to pick it up.<br><br><div class="keysContainer"><div class="key" style="width: 70% !important;">Space Bar</div></div>`,
    );
...
function showInstructions15() {
  ...
  setTimeout(() => {
    ...
    $("#instructions-content").html(
      `Finding new teammate...<div id='tutorial-loading-circle'></div>`,
    );
...
function showInstructions16() {
  $("#instructions-content").html(
    `You have been paired with a <span class='text-highlight'>robot</span> teammate!`,
  );
  $("#instructions-button").prop("disabled", false);
}
...
function showInstructions17() {
  ...
  setTimeout(() => {
    ...
    $("#instructions-content").css("display", "initial");
    $("#instructions-content").html(
      `<video controls style="width:100%;"><source src='video/tutorial_2.webm' type='video/webm' /></video>`,
    );
    ...
    $instructionsModal[0].style.setProperty("width", "100%", "important");
...
function showInstructions20() {
  ...
  setTimeout(() => {
    $("#instructions-heading").text("");
    $("#instructions-content").html(
      `<video controls style="width:100%;"><source src='video/tutorial_3.webm' type='video/webm' /></video>`,
    );
...
function showInstructions24() {
  ...
  setTimeout(() => {
    ...
    $("#instructions-content").html(
      `You found ${human.tempTargetsFound.gold.length} coin(s) and added them to your ${log[agentNum - 1][intervalCount - 1].decision} score.`,
    );
    $("#instructions-content").html(
      `Since both you and your teammate collaborated and integrated your coins into the team score, ${currentTeamScore} coins were added to the team score!`,
    );
    $("#instructions-content").css("display", "initial");
...
function showInstructions26() {
  ...
  setTimeout(() => {
    ...
    $("#instructions-heading").text("Scoring:");
    $("#instructions-content").html(
      `You found ${human.tempTargetsFound.gold.length} coin(s) and added them to your ${log[agentNum - 1][intervalCount - 1].decision} score. Since you and your teammate did not collaborate and added to your individual scores, ${currentHumanScore} point(s) were added to your individual score, ${currentTeammateScore} point(s) were added to your teammate's individual score, and 0 points were added to the team score.`,
    );
    $("#instructions-content").css("display", "initial");
    $("#instructions-content").css("margin-bottom", "2rem");
    $("#instructions-button").css("display", "inline-block");
...
function showInstructions27() {
  ...
  setTimeout(() => {
    ...
    $("#instructions-heading").text(
      "One adds to the team score and the other to the individual score:",
    );
    $("#instructions-content").html(
...
function validateUser() {
  if (
    $("#intervalSurvey").serialize() ==
    "endTutorialQuizQ1=Team%20score%3A%2012%2C%20Your%20individual%20score%3A%200%2C%20Human%20teammate%20individual%20score%3A%200&endTutorialQuizQ2=Team%20score%3A%200%2C%20Your%20individual%20score%3A%202%2C%20Human%20teammate%20individual%20score%3A%203&endTutorialQuizQ3=Team%20score%3A%200%2C%20Your%20individual%20score%3A%202%2C%20Human%20teammate%20individual%20score%3A%200"
  ) {
...
function showTrustPrompt() {
  ...
  $("#popupRoundDetails").text(
    `You picked ${human.tempTargetsFound.gold.length} coin(s).`,
  );
...
function showExploredInfo() {
  ...
  if (
    ...
    fakeAgentScores[fakeAgentNum].addedTo == "team"
  ) {
    ...
    log[agentNum - 1][intervalCount - 1].decision == "team" &&
    fakeAgentScores[fakeAgentNum].addedTo == "individual"
  ) {
    totalTeammateScore += currentTeammateScore;
    currentTeammateScore = 0;
    currentTeamScore = 0;
  } else if (
    log[agentNum - 1][intervalCount - 1].decision == "individual" &&
    fakeAgentScores[fakeAgentNum].addedTo == "team"
  ) {
...
async function animateFormula() {
  ...
  $("#formulaContainer")
    .find("*")
    .removeClass("animate__fadeIn animate__fadeOut");
  ...
  $("#formulaHeading").html(
    `You picked <span class="text-highlight">${human.tempTargetsFound.gold.length} coin(s)</span> in this round and added them to ${log[agentNum - 1][intervalCount - 1].decision == "team" ? 'the <span class="text-highlight">team' : 'your <span class="text-highlight">individual'} score</span>`,
  );
  $("#formulaHeading").toggleClass("animate__fadeIn animate__fadeOut");
  await sleep(1000 * timescale);

  // show human score
  if (log[agentNum - 1][intervalCount - 1].decision == "team")
    $("#humanIndScoreFormula").html(`${human.tempTargetsFound.gold.length}`);
  else
    $("#humanIndScoreFormula").html(
      `<img src="img/no-sign.svg" style="width: 5rem; height 5rem;">`,
    );
  ...
  await sleep(800 * timescale);
  $("#formulaHeading").html(
    `Your teammate picked <span class="text-highlight">${fakeAgentScores[fakeAgentNum - 1].gold} coin(s)</span> in this round and added them to ${fakeAgentScores[fakeAgentNum - 1].addedTo == "team" ? 'the <span class="text-highlight">team' : 'their <span class="text-highlight">individual'} score</span>`,
  );
  $("#formulaHeading").toggleClass("animate__fadeIn animate__fadeOut");
  await sleep(1000 * timescale);
  ...
  if (fakeAgentScores[fakeAgentNum - 1].addedTo == "team")
    $("#teammateIndScoreFormula").html(
      `${fakeAgentScores[fakeAgentNum - 1].gold}`,
    );
  else
    $("#teammateIndScoreFormula").html(
      `<img src="img/no-sign.svg" style="width: 5rem; height 5rem;">`,
  ...
  if (
    ...
    $("#formulaHeading").html(
      `The team score for this round is ${currentTeamScore}!`,
    );
  } else {
...
async function animateScores() {
  ...
  $("#humanIndPiggyBankText").html(
    `${totalHumanScore} &times; <img src="img/coin.svg" style="width: 1.5em; height: 1.5em; vertical-align: middle;">`,
  );
  $("#teammateIndPiggyBankText").html(
    `${totalTeammateScore} &times; <img src="img/coin.svg" style="width: 1.5em; height: 1.5em; vertical-align: middle;">`,
  );
  $("#teamPiggyBankText").html(
    `${totalTeamScore} &times; <img src="img/coin.svg" style="width: 1.5em; height: 1.5em; vertical-align: middle;">`,
  );

  if (totalHumanScore > prevTotalHumanScore) {
    $("#humanIndPiggyBankText").css(
      "filter",
      "drop-shadow(0 2px 10px #F6BE00)",
    );
    $("#humanIndPiggyBankText").addClass(
      "animate__animated animate__pulse animate__infinite",
    );
  } else {
    $("#humanIndPiggyBankText").css("filter", "initial");
    $("#humanIndPiggyBankText").removeClass(
      "animate__animated animate__pulse animate__infinite",
    );
  }

  if (totalTeammateScore > prevTotalTeammateScore) {
    $("#teammateIndPiggyBankText").css(
      "filter",
      "drop-shadow(0 2px 10px #F6BE00)",
    );
    $("#teammateIndPiggyBankText").addClass(
      "animate__animated animate__pulse animate__infinite",
    );
  } else {
    $("#teammateIndPiggyBankText").css("filter", "initial");
    $("#teammateIndPiggyBankText").removeClass(
      "animate__animated animate__pulse animate__infinite",
    );
  }
  ...
  if (totalTeamScore > prevTotalTeamScore) {
    ...
    $("#teamPiggyBankText").addClass(
      "animate__animated animate__pulse animate__infinite",
    );
  } else {
    $("#teamPiggyBankText").css("filter", "initial");
    $("#teamPiggyBankText").removeClass(
      "animate__animated animate__pulse animate__infinite",
    );
  }
  ...
  if (!engineInited) {
    ...
    smallBucketHeight =
      document.querySelector("#humanIndPiggyBank").scrollHeight;
  ...
  members.forEach((member) => {
    if (member.id == "victim" && member.isFound) {
      $map.drawImage({
        ...
        x:
          grid[member.loc].x * boxWidth + boxWidth / 2 - victimMarker.width / 2,
        y: grid[member.loc].y * boxHeight + boxHeight / 2 - victimMarker.height,
      });
...
function scaleImages() {
  ...
  $botImage
    .parent()[0]
    .scroll(
      (botLeft + (botRight - botLeft + 1) / 2) * ($botImage.width() / columns) -
        $(".explored").width() / 2,
      (botTop + (botBottom - botTop + 1) / 2) * ($botImage.height() / rows) -
        $(".explored").height() / 2,
    );
  $humanImage
    .parent()[0]
