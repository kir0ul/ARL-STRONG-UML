...
function Gapless5Source(parentPlayer, parentLog, inAudioPath) {
  ...
  this.setCrossfade = (amountIn, amountOut, resetEndedCallback = true) => {
    ...
    if (endpos > 0) {
      if (crossfadeIn + crossfadeOut > endpos) {
        log.warn(
          `Crossfades add up to longer than duration (${crossfadeIn + crossfadeOut} > ${endpos}), clamping`,
        );
  ...
  const setEndedCallbackTime = (restSecNormalized) => {
    ...
    if (this.inPlayState(true)) {
      ...
      log.debug(
        `onEnded() will be called on ${this.audioPath} in ${restSec.toFixed(2)} sec`,
      );
    ...
    if (buffer !== null) {
      ...
      player.context.resume().then(() => {
        if (state === Gapless5State.Starting) {
          ...
          log.debug(
            `Playing WebAudio${looped ? " (looped)" : ""}: ${this.audioPath} at ${offsetSec.toFixed(2)} sec`,
          );
      ...
      audio
        ...
        .then(() => {
          if (state === Gapless5State.Starting) {
            log.debug(
              `Playing HTML5 Audio${looped ? " (looped)" : ""}: ${this.audioPath} at ${offsetSec.toFixed(2)} sec`,
            );
            setState(Gapless5State.Play);
            player.onplay(this.audioPath);
